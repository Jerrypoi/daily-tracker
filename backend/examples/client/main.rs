...
fn main() {
    ...
    match matches.get_one::<String>("operation").map(String::as_str) {
        ...
        Some("CreateDailyTrack") => {
            let result = rt.block_on(client.create_daily_track(
                  ???
            ));
        ...
        */
        Some("GetDailyTracks") => {
            let result = rt.block_on(client.get_daily_tracks(
                  None,
                  None,
                  Some(789)
            ));
        ...
        Some("GetDailyTrackById") => {
            let result = rt.block_on(client.get_daily_track_by_id(
                  789
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
        /* Disabled because there's no example.
        Some("CreateTopic") => {
            let result = rt.block_on(client.create_topic(
                  ???
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
        */
        ...
        Some("GetTopicById") => {
            let result = rt.block_on(client.get_topic_by_id(
                  789
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
